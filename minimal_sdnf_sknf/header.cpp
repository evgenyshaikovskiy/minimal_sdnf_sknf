...
vector<string> FragmentationAndAnother(string sourceString)
{
	vector<string> result;
...
string CheckWhetherNeighboursSDNF(string s1, string s2)
{
...
string GluingSDNF(string sourceString)
{
	...
	for (int i = 0; i < fragments.size(); i++)
	{
		...
		{
			if (CheckWhetherNeighboursSDNF(fragments[i], fragments[j]) != "")
			{
				if (resultOfGluing == "")
				{
					resultOfGluing += CheckWhetherNeighboursSDNF(fragments[i], fragments[j]);
				}
...
{
	string gluingResult = "";
	vector<string> fragments = FragmentationAndAnother(sourceString);
...
{
	vector<string> r = FragmentationAndAnother(sourceString);
...
{
	vector<string> h = FragmentationAndAnother(SKNF);
	vector<string> l = FragmentationAndAnother(KNF);
	vector<vector<char>> table;
	vector<string> r;
	table.resize(l.size());
...
bool isInclude4(string s, string str) {
	vector<string> Str = FragmentationAndAnother(str);
	for (int i = 0; i < Str.size(); i++)
	{
